<?php
$nameErr = $emailErr = $phoneErr = $websiteErr = $genderErr = $passwordErr = $confirmPasswordErr = $termsErr = "";
$name = $email = $phone = $website = $comment = $gender = $password = $confirmPassword = $terms = "";
$submitted = false;
$attemptCount = 0; // Initialize counter at 0

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $submitted = true;

    // Increment the counter based on the hidden field's previous value
    if (isset($_POST["attempt_count"])) {
        $attemptCount = (int)$_POST["attempt_count"] + 1;
    } else {
        $attemptCount = 1; 
    }
...
$formValid = $submitted && empty($nameErr) && empty($emailErr) && empty($phoneErr) && empty($websiteErr) && empty($passwordErr) && empty($confirmPasswordErr) && empty($genderErr) && empty($termsErr);
?>
...
<div class="form-container">
    <h2>Get in Touch</h2>
    <p class="attempt-counter">Submission attempt: <?= $attemptCount ?></p>
    ...
    <form method="post" action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) ?>">
        ...
        <div class="field-row">
            ...
            <input type="text" id="phone" name="phone" placeholder="+1 555-0198" value="<?= $phone ?>">
        ...
        <div class="field-row">
            <div class="checkbox-item">
                <input type="checkbox" id="terms" name="terms" value="yes" <?= ($terms == "checked") ? "checked" : "" ?>>
